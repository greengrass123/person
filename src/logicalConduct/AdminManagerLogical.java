...
public class AdminManagerLogical  extends HttpServlet {
	...
	//管理员退出
	public void adminLoginOut(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
	...
	//点击审核、未审核显示对应状态广告
	public void aduitInfo(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {		
		response.setContentType("text/html;charset=UTF-8");
		request.setCharacterEncoding("UTF-8");
		...
		if(audit.equals("未审核"))
		{
			...
			{	 
			 ...
			 if(!pasteName.equals("所有粘贴栏"))  //按粘贴栏展示 
			 {
			    ...
			    {
			    	String pasteStr= od.query_adPostBypicId(noauditlist.get(i).getPicId());
		...
		}else if(audit.equals("已审核"))
		{
			...
			if(adType.equals("所有广告"))
			{
			 ...
			 if(!pasteName.equals("所有粘贴栏"))  //按粘贴栏展示 
			 {
			    ...
			    {
			    	String pasteStr= od.query_adPostBypicId(auditlist.get(i).getPicId());
			    	if(!pasteName.equals(pasteStr)){
			    		System.out.println("删除一个数据----->");
			    		auditlist.remove(i);
			    	}
	...
	public void auditnoBy(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		int adId=Integer.parseInt(request.getParameter("adId"));
		data=new AdminLogic();
		...
		if(userName.equals("")||userPasswd.equals("")){
			...
			if(userPasswd.equals(passwordConfirm)){
				if(!userName.equals(oldname)){//如果更改了用户名
					...
					if(f==true){
						boolean flag=data.updateUser(userId,userName,userPasswd,email,phone);
						if(flag==true){
							user.setUserName(userName);
							user.setPassword(userPasswd);
							user.setEmail(email);
	...
	//显示粘贴栏信息
	public void pasteShow(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
		List list=data.get_paste();
		request.setAttribute("list",list);
		System.out.println("t4");
		request.getRequestDispatcher("pasteManage.jsp").forward(request,response);		 
	}
	
	//插入一个粘贴栏
	public void insertPaste(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
		response.setContentType("text/html;charset=UTF-8"); 
		request.setCharacterEncoding("UTF-8"); 	 
	...
	//修改粘贴栏信息
	public void updatePaste(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
		response.setContentType("text/html;charset=UTF-8"); 
		request.setCharacterEncoding("UTF-8"); 		
	...
	//删除粘贴栏
	public void delPaste(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
	...
	//后台显示单位
	public void unitShow(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
		...
		request.getRequestDispatcher("unitManager.jsp").forward(request,response);
	}
	
	//插入一个单位
	public void insertUnit(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
	...
	//显示类别信息
	public void typeShow(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
	...
	//显示粘贴栏类别信息
	public void pasteTypeShow(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
		List list=data.get_pasteType();
		request.setAttribute("list",list);
		request.getRequestDispatcher("pasteTypeManager.jsp").forward(request,response);
	}
	
	//插入一个粘贴栏类别
	public void insertPasteType(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
		response.setContentType("text/html;charset=UTF-8"); 
		request.setCharacterEncoding("UTF-8"); 
	...
	//修改粘贴栏类别
	public void updatePasteType(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
		response.setContentType("text/html;charset=UTF-8"); 
		request.setCharacterEncoding("UTF-8"); 
	...
	//删除粘贴栏类别
	public void delPasteType(HttpServletRequest request, HttpServletResponse response)
	throws ServletException, IOException {
		data=new AdminLogic();
		String typeId=request.getParameter("typeId");
		System.out.println("typeId="+typeId);
